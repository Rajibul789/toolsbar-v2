...
import { Plus, Tag, Trash2, X, Save, RefreshCw } from "lucide-react";
import { toast } from "sonner";

function slugify(s: string) { return s.toLowerCase().trim().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""); }

interface TagItem { id: string; name: string; slug: string; _count?: { posts: number }; }
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {tags.map((tag) => (
          <div key={tag.id} className="flex items-center gap-2 pl-3 pr-2 py-1.5 rounded-full group transition-all"
            ...
            <span className="text-[10px] font-mono text-text-muted">{tag._count?.posts ?? 0}</span>
        ...
        {!loading && tags.length === 0 && (
          <p className="text-sm font-mono text-text-muted py-4">No tags yet. Add your first tag above.</p>
        )}
      </div>
          ...
          {tags.map((tag, i) => (
            <div key={tag.id} className="grid grid-cols-3 gap-4 px-5 py-3 items-center group border-b last:border-0"
              ...
              <div className="flex items-center justify-end gap-3">
                <span className="text-xs font-mono text-text-muted">{tag._count?.posts ?? 0}</span>
                <button onClick={() => deleteTag(tag.id, tag.name)}
