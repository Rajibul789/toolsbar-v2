...
export function BlogSearch({ posts }: { posts: PublicBlogPost[] }) {
            ...
            {results.length > 0 ? (
              <>
                ...
                {results.slice(0, 6).map((post) => (
                  ...
                  >
                    <span className="text-sm font-mono text-text-primary hover:text-neon-cyan transition-colors line-clamp-1">
                      {post.title}
                    </span>
                    <div className="flex items-center gap-3 text-[11px] font-mono text-text-muted">
                      {post.category && (
                        <span className="text-neon-cyan/60">{post.category.name}</span>
                      )}
